...
class RoomModel extends Model{
    ...
    public function getAvailableRoom() {
        ...
        $this->getDay();
        $bid = $_GET['bid'];
        $sql = "SELECT DISTINCT r.roomnumber as roomnumber, r.rid FROM building b, room r WHERE r.rid not in ( SELECT c.rid from course c WHERE c.time <= '".$this->time."' and c.endtime >= '".$this->time."' and c.days like '%".$this->day."%') and r.bid = ".$bid;
        $sqlArray = $this->db->getAll($sql);
    ...
    public function getAvailableRoomNumber(){
        ...
        $sql = "SELECT r.rid,b.name, r.roomnumber, COUNT(b.bid) as count, b.bid FROM building b, room r WHERE r.rid not in ( SELECT c.rid from course c WHERE c.time <= '".$this->time."' and c.endtime >= '".$this->time."' and c.days like '%".$this->day."%') and b.bid = r.bid GROUP BY b.bid";
        $sqlArray = $this->db->getAll($sql);
